          ...
          <Modal v-model="modal" width="420"
            ...
            <div class="userLine"><span>用户权限</span>
              <Select v-model="userPrivilige" style="width:200px;">
                <Option v-for="(item,index) in privileges" :value="item.value" :key="item.value">{{ item.label }}
                </Option>
              </Select><a class="modify_btn" @click="modifyP">修改</a></div>
            <div class="userLine"><span>体验时长</span><Input v-model="userTime" style="width:200px;"></Input><a
              class="modify_btn" @click="modifyT">修改</a></div>
            <div class="userLine"><span>修改密码</span><Input placeholder="请输入要重置的密码" v-model="userPassword"
                                                          style="width:200px;"></Input><a class="modify_btn"
                                                                                          @click="modifyPass">修改</a>
            </div>
    ...
    methods: {
      ...
      modifyPass() {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.$http.put("http://112.74.25.26/user/password", {
              "userid": this.userId,
              "password": pass
            }).then((res) => {
      ...
      confirmAddRes() {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.$http.post("http://112.74.25.26/user/infoList", param).then((res) => {
                this.modal3 = false;
                this.modal2 = false;
      ...
      getUserinfoList(userid) {
        ...
        this.$http.get("http://112.74.25.26/user/infoList", {params: param}).then((res) => {
            this.data3 = [];
