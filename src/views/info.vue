<template>
  <div class="infowrap">
    ...
    <Modal v-model="modal" width="90%" class-name="vertical-center-modal" :closable="false">
      <iframe :src="resultUrl" width="100%" height="100%" style="overflow: auto"></iframe>
    </Modal>
    <div class="videoBox">
      <div class="cvideocont_l">
        <ul class="r_list">
          <li class="r_list_li" v-for="(item,index) in localData.info.resultList1" :key="index">
            <img src="http://media.tianxingzhe.vip/network/word.png">
            <h3>{{item.title}}</h3>
          </li>
          <li v-for="(item,ind) in resultList" :key="ind + localData.info.resultList1.length">
            <img src="http://media.tianxingzhe.vip/network/word.png" @click="showItem(item)">
            <h3>{{item.title}}</h3>
          </li>
        </ul>
      </div>
      <div class="cvideocont_r">
        <div>
          <div>
            <img src="http://media.tianxingzhe.vip/network/network_icon.png">
          </div>
          <h4>{{username}}用户</h4>
        </div>
      </div>
...
.infowrap {
  width: 100%;
  height: 100%;
  background: url("http://media.tianxingzhe.vip/network/net_bg.jpg") no-repeat
    center center;
