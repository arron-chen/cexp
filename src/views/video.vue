        ...
        <div class="cvideocont_r">
          <div class="r_title">
            <Select v-model="model8" clearable style="width:200px" v-on:on-change="onchange">
              <Option v-for="item in localData.video.cityList" :value="item.value" :key="item.value">{{ item.label }}</Option>
            </Select>
          </div>
          <ul class="r_list">
            <li v-for="item in listU" :key="item.id" @click="handleClickItem(item)"><img :src="item.imgSrc"><span>{{item.text}}</span></li>
          </ul>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.listIndex=localData.video.lists.a;
    },
    methods:{
      backto(){
        this.$router.push({
